<?
//定义应用所需常量

//近似查询接口相关配置
//define('SEARCH_URL', 'http://searchapi.chofn.net/');
define('SEARCH_URL', 'http://tmsearch.chofn.api/');
define('SEARCH_KEY', '89eb637c610f94b9d281c458bca42421');

//用户登录URL
define('UC_URL', 'http://my.uzichan.cn/');
//出售者平台
define('SELLER_URL', 'http://seller.uzichan.cn/');
//一只蝉URL
define('SITE_URL', 'http://www.uzichan.cn/');
//一移动版URL
define('WAP_URL', 'http://m.uzichan.cn/');
//交易站后台URL
define('TRADE_URL', 'http://master.uzichan.cn');
define('TRADE_STATIC', 'http://master.uzichan.cn/');
//脱兔URL
define('GUANJIA_URL', 'http://m.tuotu.net/');
//文章发布后台地址主要针对图片
define('WM_URL', 'http://wm.chofn.net/');
//CRM地址URL
define('CRM_URL', 'http://demo.chofn.com:88/');
//蝉检测
define('CHECK_URL', 'http://ce.chanshu.im/');
//商标注册
define('TM_URL', 'http://r.chaofan.wang/reg/');
//专利注册
define('PT_URL', 'http://pt.chofnipr.com/');
//一只蝉前台静态CDN地址
define('YZC_STATIC', ''.StaticDir);
//一只蝉后台静态CDN地址
define('TR2_STATIC', TRADE_URL);
//镖局
define('BJ_URL', 'http://www.biaoju01.com/');

define('TEL400', '4006-762-003');
?>